...
import {
    ...
    Plus, Trash2, Eye, BookOpen, Play, UserCheck, Info, Sliders, Check,
    SaudiRiyalIcon
} from 'lucide-react';
...
function SettingsPageInner() {
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 md:px-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
            ...
            <nav className="grid w-full grid-cols-1 md:grid-cols-2 gap-3 rounded-[1.25rem] border border-slate-200/70 bg-[linear-gradient(135deg,#f8fafc_0%,#eef2f7_100%)] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] md:flex md:w-fit md:flex-wrap md:items-center">
                {[
                    ...
                    {[
                        ...
                        <div key={group.category} className="border border rounded-3xl overflow-hidden mb-4">
                            ...
                            {!collapsedCategories[group.category] && (
                                <div className="p-3 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 bg-card">
...
function SiteControlTab({ localSettings, updateSettings, t }: TabProps) {
    ...
    const CONTROL_ITEMS = [
        ...
        { id: 'show_agents_all_departments_access', label: 'دخول الوسطاء لجميع الأقسام', type: 'uiFlag', subcategoryId: 'user_page' },
        { id: 'enable_global_free_trial', label: 'تفعيل التجربة المجانية (للجميع)', type: 'uiFlag', subcategoryId: 'user_page' },
    ...
    const UiFlagRow = ({ id, label }: { id: string; label: string }) => {
        ...
        const isToggled = !!localSettings.uiFlags[id];
        
        // Handle department selections specifically for show_agents_all_departments_access
        const showDeptSelector = id === 'show_agents_all_departments_access' && isToggled;
        const currentAllowedDepts = (localSettings.texts?.agents_accessible_departments || '')
            .split(',')
            .map((d: string) => d.trim())
            .filter(Boolean);
            
        const toggleDept = (deptVal: string) => {
            let nextDepts;
            if (currentAllowedDepts.includes(deptVal)) {
                nextDepts = currentAllowedDepts.filter((d: string) => d !== deptVal);
            } else {
                nextDepts = [...currentAllowedDepts, deptVal];
            }
            updateSettings({
                texts: {
                    ...(localSettings.texts || {}),
                    agents_accessible_departments: nextDepts.join(',')
                }
            });
        };

        const depts = [
            { value: 'properties', label: language === 'ar' ? 'إدارة الأملاك' : 'Property Management' },
            { value: 'offers', label: language === 'ar' ? 'العروض' : 'Offers' },
            { value: 'orders', label: language === 'ar' ? 'الطلبات' : 'Orders' },
            { value: 'marketing', label: language === 'ar' ? 'التسويق' : 'Marketing' },
            { value: 'legal', label: language === 'ar' ? 'الإدارة القانونية' : 'Legal Department' },
            { value: 'finance', label: language === 'ar' ? 'الإدارة المالية' : 'Finance Department' },
            { value: 'employees', label: language === 'ar' ? 'إدارة الموظفين' : 'Employee Management' },
        ];
        ...
        return (
            <div className="space-y-3">
                <div className="rounded-[1rem] border border bg-card shadow-sm hover:border-slate-300 transition-all flex items-center justify-between px-5 py-4">
                    ...
                    <button
                        type="button"
                        onClick={() => updateSettings({ uiFlags: { ...(localSettings.uiFlags || {}), [id]: !isToggled } })}
                        className={`w-14 h-7 rounded-full relative transition-all shrink-0 shadow-inner border border-transparent ${isToggled ? 'bg-emerald-500 border-emerald-600' : 'bg-muted border-slate-300'}`}
                    >
                        <div className={`absolute top-1 w-5 h-5 rounded-full bg-card transition-all shadow-sm ${isToggled ? 'left-8' : 'left-1'}`} />
                    </button>
                </div>
                
                {showDeptSelector && (
                    <div className="mr-4 p-4 rounded-[1rem] border bg-muted/30 space-y-3">
                        <p className="text-xs font-black text-slate-500">
                            {language === 'ar' ? 'اختر الأقسام التي يسمح للوسطاء بدخولها:' : 'Select departments accessible by agents:'}
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                            {depts.map(dept => {
                                const active = currentAllowedDepts.includes(dept.value);
                                return (
                                    <button
                                        key={dept.value}
                                        type="button"
...
function LogsModal({ isOpen, onClose }: { isOpen: boolean, onClose: (v: boolean) => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative bg-card rounded-[1rem] w-[95vw] sm:max-w-2xl p-4 sm:p-8 shadow-2xl overflow-hidden">
...
function CommissionModal({ isOpen, onClose }: { isOpen: boolean, onClose: (v: boolean) => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <div onClick={() => onClose(false)} className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" />
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative bg-card rounded-[1rem] w-[95vw] sm:max-w-lg p-4 sm:p-8 shadow-2xl">
                <div className="flex items-center justify-between mb-8">
