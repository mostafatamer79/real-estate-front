...
const getStatusConfig = (req: LegalRequest, t: (k: string) => string) => {
  if (!req.invoiceSent)
    return { label: t("legal.requests.awaitingInvoice"), dot: "bg-card/20", text: "text-white/40", ring: "border-white/10", pill: "bg-card/5" };
  if (req.clientDecision === "accepted")
    return { label: t("legal.decision.accepted"),        dot: "bg-card",     text: "text-white/80", ring: "border-white/20", pill: "bg-card/5" };
  if (req.clientDecision === "rejected")
    return { label: t("legal.decision.rejected"),        dot: "bg-card/30",  text: "text-white/40", ring: "border-white/10", pill: "bg-card/5" };
  return   { label: t("legal.decision.pending"),         dot: "bg-card/60",  text: "text-white/70", ring: "border-white/20", pill: "bg-card/5" };
};
  ...
  return (
    <div className={`${embedded ? '' : 'min-h-screen bg-slate-950 text-white pb-12'}`} dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="hidden md:flex sticky top-16 z-20 bg-slate-950 border-b border-white/10">
        <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 h-8 px-4 rounded-full bg-card text-slate-950 text-[10px] font-black uppercase tracking-widest hover:bg-card/90 transition-colors"
          >
      ...
      <div className={`${embedded ? '' : 'relative z-10 max-w-3xl mx-auto px-6 py-10'}`}>
        ...
        {/* Loading */}
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-20 sm:py-40 gap-3 md:gap-6">
        ...
        {/* Error */}
        {!isLoading && error && (
          <div className="flex flex-col items-center justify-center py-20 sm:py-40 gap-4">
            <div className="w-14 h-14 rounded-2xl border border-white/10 flex items-center justify-center">
        ...
        {!isLoading && !error && requests.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 sm:py-40 gap-4 md:gap-8 text-center">
            ...
            <Link
              href="/services/form?type=legal"
              className="flex items-center gap-2 h-11 px-6 rounded-2xl bg-card text-slate-950 text-xs font-black uppercase tracking-widest hover:bg-card/90 transition-colors"
            >
              ...
              {requests.map((req, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-card/[0.03] border border-white/10 hover:border-white/20 rounded-[1.25rem] overflow-hidden transition-all duration-300"
                  >
                    ...
                    <div className="p-5 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4 min-w-0">
                        <div className="w-11 h-11 rounded-2xl bg-card/5 border border-white/10 flex items-center justify-center shrink-0">
                      ...
                      <div className="flex items-center gap-2.5 shrink-0">
                        ...
                        <button
                          ...
                          className="w-10 h-10 sm:w-7 sm:h-7 rounded-full border border-white/10 hover:border-white/20 flex items-center justify-center transition-colors"
                        >
                          {isExpanded
                      ...
                      {isExpanded && (
                        ...
                        >
                          <div className="border-t border-white/[0.06] px-5 py-5 space-y-4">
                                ...
                                {[req.firstParty && { label: t("disputes.firstParty"), party: req.firstParty }, req.secondParty && { label: t("disputes.secondParty"), party: req.secondParty }]
                                  ...
                                  <div key={item.label} className="p-4 bg-card/[0.02] rounded-2xl border border-white/[0.06]">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <div className="w-7 h-7 rounded-lg bg-card/5 border border-white/10 flex items-center justify-center shrink-0">
                            ...
                            {/* Description */}
                            {req.description && (
                              <div className="p-4 bg-card/[0.02] rounded-2xl border border-white/[0.06]">
                            ...
                            {req.invoiceSent && req.invoicePrice != null && (
                              <div className="p-5 bg-card/[0.03] rounded-2xl border border-white/10 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  <div className="w-9 h-9 rounded-xl bg-card/5 border border-white/10 flex items-center justify-center">
                            ...
                            {req.invoiceSent && req.clientDecision === "pending" && (
                              <div className="flex gap-3 pt-1">
                                <motion.button
                                  ...
                                  className="flex-1 h-12 rounded-2xl bg-card text-slate-950 font-black text-sm flex items-center justify-center gap-2 hover:bg-card/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                ...
                                <motion.button
                                  ...
                                  disabled={actionLoading === req.id}
                                  className="flex-1 h-12 rounded-2xl bg-card/5 border border-white/10 hover:border-white/20 text-white/70 font-black text-sm flex items-center justify-center gap-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                            ...
                            <AnimatePresence>
                              {actionMessage?.id === req.id && (
                                <motion.p
                                  ...
                                  className="text-center text-xs font-bold p-3 rounded-2xl border border-white/10 bg-card/[0.03] text-white/50"
                                >
                                  {actionMessage.text}
                                </motion.p>
                              )}
