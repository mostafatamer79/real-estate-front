...
export default function Footer() {
  ...
  return (
    ...
    >
      ...
      {/* ════════════════════════════════════════════════════
          DESKTOP LAYOUT (Unchanged 100% original design & colors)
          ════════════════════════════════════════════════════ */}
      ...
      {/* ════════════════════════════════════════════════════
          MOBILE LAYOUT (Optimized, Simple, Clean & Native-feel)
          ════════════════════════════════════════════════════ */}
      <div className="md:hidden relative z-10 px-5 pt-6 pb-6 space-y-5">
        {/* Brand & Social Strip */}
        <div className="flex items-center justify-between">
          <Link href="/" className="inline-block active:opacity-80 transition-opacity">
            <img
              src={settings?.logoWhiteUrl || "/icons/white.png"}
              alt={t("footer.logo_alt")}
          ...
          {/* Contact Action Circles (Email, Phone, X) */}
          <div className="flex items-center gap-2">
            {settings.contactEmail && (
              <a
                href={`mailto:${settings.contactEmail}`}
                aria-label="Email"
                className="h-9 w-9 rounded-full flex items-center justify-center bg-slate-900 border border-slate-800 text-slate-400 active:text-white active:bg-slate-800 active:scale-95 transition-all shadow-sm"
              >
        ...
        {/* Quick Properties Shortcuts (Bayut / Aqar mobile style) */}
        <div className="grid grid-cols-2 gap-2.5">
          <Link
            href="/properties?purpose=sale"
            className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-900/90 border border-slate-800/80 active:bg-slate-800 transition-colors"
          >
          ...
          >
            ...
            <span className="text-xs font-semibold text-slate-200">
              {isAr ? "عقارات للإيجار" : "For Rent"}
            </span>
          </Link>
        </div>

        {/* Structured 2-Column Links */}
        <div className="grid grid-cols-2 gap-4 pt-1">
          {/* Platform Column */}
          <div className="space-y-2">
            <h4 className="text-[0.72rem] font-bold uppercase tracking-wider text-slate-500 pb-1 border-b border-slate-800/60">
              {t("footer.about_platform")}
            </h4>
            <div className="space-y-1">
              {platformLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="flex items-center justify-between py-1.5 text-[0.82rem] font-medium text-slate-400 active:text-white transition-colors"
                >
                  <span className="truncate">{link.label}</span>
                  <ChevronIcon className="w-3 h-3 text-slate-600 shrink-0" />
                </Link>
              ))}
            </div>
          </div>
        ...
        {/* Clean Bottom Copyright & Tag */}
        <div className="pt-3 border-t border-slate-800/80 flex flex-col items-center gap-1 text-center">
